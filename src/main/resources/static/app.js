...
function refreshTimeTable() {
  $.getJSON("/timeTable", function (timeTable) {
    ...
    const allTeachers = $("#allTeachers");
    allTeachers.children().remove();
      ...
      rowByTeacher
        .append($(`<th class="align-middle"/>`)
          .append($("<span/>").text(`
                ...
                `)));
      $.each(teacherList, (index, teacher) => {
        rowByTeacher.append($("<td/>").prop("id", `timeslot${timeslot.id}teacher${convertToId(teacher.name)}`));
      });
        ...
        .append($(`<div class="card-body p-2"/>`)
          .append($(`<h5 class="card-title mb-1"/>`).text(lesson.subject))
            .append($(`<p class="card-text ml-2 mb-1"/>`)
                .append($(`<em/>`).text( (lesson.teacher != null ? `by ${lesson.teacher.name}` : "no teacher assigned"))))
    ...
    $.each(timeTable.teacherList, (index, teacher) => {
          const color = pickColor(teacher.name);
          const teacherElementWithoutDelete = $(`<div class="card" style="background-color: ${color}"/>`)
            .append($(`<div class="card-body p-2"/>`)
              .append($(`<h5 class="card-title mb-1"/>`).text(teacher.name)));
          const teacherElement = teacherElementWithoutDelete.clone();
//          teacherElement.find(".card-body").prepend(
//            $(`<button type="button" class="ml-2 btn btn-light btn-sm p-1 float-right"/>`)
//              .append($(`<small class="fas fa-trash"/>`)
//              ).click(() => deleteTeacher(teacher))
//          );
          allTeachers.append(teacherElement);
        });
  });
}
